(** Administrative lemmas for FuncColour.
...
Require Export FuncColour.Fsub_LetSum_Infrastructure.
...
Lemma typing_regular : forall E Q e T,
  typing E Q e T ->
  wf_env E /\ expr e /\ wf_qtyp E T /\ wf_qua E Q.
Proof with simpl_env; try solve [auto | intuition auto].
  intros E Q e T H; induction H...
  Case "typing_var".
    repeat split...
    eauto using wf_qtyp_from_binds_typ.
  Case "typing_abs".
    pick fresh y.
    destruct (H0 y) as [Hok [J [K M]]]...
    repeat split... inversion Hok...
    SCase "Second of original three conjuncts".
      pick fresh x and apply expr_abs;
          eauto using qual_from_wf_qua, qtype_from_wf_qtyp, wf_qtyp_from_wf_env_typ.
        destruct (H0 x)...
        rewrite_env (empty ++ y ~ bind_typ V ++ E) in M. 
        eapply wf_qua_strengthening_typ in M...
    SCase "Third of original three conjuncts".
  ...
  Case "typing_app".
    repeat split...
    destruct IHtyping1 as [_ [_ [K M]]].
    inversion K; subst...
    inversion select (wf_typ _ (typ_arrow _ _)); subst...
  Case "typing_tabs".
    pick fresh Y.
    destruct (H0 Y) as [Hok [J [K M]]]...
    inversion Hok; subst.
    ...
    SCase "Second of original three conjuncts".
      pick fresh X and apply expr_tabs;
        ...
        qual_from_wf_qua, qtype_from_wf_qtyp, wf_qtyp_from_wf_env_typ...
        destruct (H0 X)...
    SCase "Third of original three conjuncts".
      rewrite_env (empty ++ E); eapply wf_qua_strengthening_sub; eauto...
  ...
  Case "typing_qabs".
    pick fresh Y.
    destruct (H1 Y) as [Hok [J [K M]]]...
    inversion Hok; subst.
    repeat split...
    SCase "Second of original three conjuncts".
      pick fresh X and apply expr_qabs; 
        eauto using type_from_wf_typ, wf_typ_from_wf_env_sub,
          qual_from_wf_qua, qtype_from_wf_qtyp, wf_qtyp_from_wf_env_typ...
        destruct (H1 X)...
    SCase "Third of original three conjuncts".
  ...
  Case "typing_subqual".
    repeat split...
    destruct (subqual_regular _ _ _ H0)...
  Case "typing_let".
    repeat split...
    SCase "Second of original three conjuncts".
...
Proof with simpl_env; try solve [auto | intuition auto].
  ...
  Case "typing_ctx_app".
    repeat split...
    destruct (typing_regular _ _ _ _ H1) as [? [? [? ? ]]]...
    inversion select (wf_qtyp _ _); subst...
    inversion select (wf_typ _ (typ_arrow _ _))...
  Case "typing_ctx_tabs".
    destruct (sub_regular _ _ _ H0) as [? [? ?]]...
    repeat split...
    econstructor...
  Case "typing_ctx_qabs".
    destruct (subqual_regular _ _ _ H0) as [? [? ?]]...
    repeat split...
    econstructor...
  Case "typing_ctx_let".
    repeat split...
    pick fresh x.
    destruct (typing_regular _ _ _ _ (H0 x ltac:(eauto)))
      as [? [? [? ?]]]...
    eapply wf_qtyp_from_wf_env_typ; eassumption.
  Case "typing_ctx_inl".
    repeat split...
    destruct IHtyping_ctx as [_ [Typ _]].
    inversion Typ; subst;
  ...
  Case "typing_ctx_inr".
    ...
    inversion Typ; subst;
      inversion select (wf_typ _ (typ_sum _ _))...
  Case "typing_ctx_case".
    repeat split...
    econstructor...
    SCase "Typing the left branch".
      pick fresh x.
      destruct (typing_regular _ _ _ _ (H1 x ltac:(eauto)))
        as [? [? [? ?]]]...     
      eapply wf_qtyp_from_wf_env_typ; eassumption.
    SCase "Typing the right branch".
      pick fresh x.
      destruct (typing_regular _ _ _ _ (H2 x ltac:(eauto)))
        as [? [? [? ?]]]...     
      eapply wf_qtyp_from_wf_env_typ; eassumption.
  Case "typing_ctx_pair_1".
    repeat split...
    destruct IHtyping_ctx as [_ [Typ _]].
    inversion Typ; subst;
      inversion select (wf_typ _ (typ_pair _ _))...
  Case "typing_ctx_pair_2".
    repeat split...  
    destruct IHtyping_ctx as [_ [Typ _]].
    inversion Typ; subst;
      inversion select (wf_typ _ (typ_pair _ _))...
  Case "typing_ctx_barrier".
    repeat split...
    destruct t...
  Case "typing_ctx_check".
    destruct IHtyping_ctx as [? [Typ ?]].
    inversion Typ...
  Case "typing_ctx_sub".
    repeat split...
    unshelve epose proof (subqtype_regular _ _ _ H0)...
...
#[export] Hint Extern 1 (wf_qua ?E ?T) =>
  match goal with
  | H: typing E T _ _ |- _ => apply (proj2 (proj2 (proj2 (typing_regular _ _ _ _ H))))
