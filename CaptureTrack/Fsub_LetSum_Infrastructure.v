(** Infrastructure lemmas and tactic definitions for Fsub.CaptureTrack.
...
Require Export Fsub.CaptureTrack.Fsub_LetSum_Definitions.
...
Fixpoint fv_qe (e : exp) {struct e} : atoms :=
  ...
  | exp_app e1 e2 Q => (fv_qe e1) `union` (fv_qe e2) `union` (fv_qq Q)
  ...
  | exp_let Q e1 e2 => (fv_qq Q) `union` (fv_qe e1) `union` (fv_qe e2)
  | exp_inl P e1 => (fv_qq P) `union` (fv_qe e1)
  | exp_inr P e1 => (fv_qq P) `union` (fv_qe e1)
  | exp_case e1 Q1 e2 Q2 e3 => (fv_qq Q1) `union` (fv_qq Q2) `union` (fv_qe e1) `union` (fv_qe e2) `union` (fv_qe e3)
...
Fixpoint fv_ee (e : exp) {struct e} : atoms :=
  ...
  | exp_bvar i => {}
  | exp_fvar x => {{ x }}
  | exp_abs P V e1 => (fv_qq P) `union` (fv_qqt V) `union` (fv_ee e1)
  | exp_app e1 e2 Q => (fv_ee e1) `union` (fv_ee e2) `union` (fv_qq Q)
  | exp_tabs P V e1 => (fv_qq P) `union` (fv_qt V) `union` (fv_ee e1)
  | exp_tapp e1 V => (fv_ee e1) `union` (fv_qt V)
  | exp_qabs P V e1 => (fv_qq P) `union` (fv_qq V) `union` (fv_ee e1)
  | exp_qapp e1 V => (fv_ee e1) `union` (fv_qq V)
  | exp_let Q e1 e2 => (fv_ee e1) `union` (fv_ee e2) `union` (fv_qq Q)
  | exp_inl P e1 => (fv_qq P) `union` (fv_ee e1)
  | exp_inr P e1 => (fv_qq P) `union` (fv_ee e1)
  | exp_case e1 Q1 e2 Q2 e3 => (fv_ee e1) `union` (fv_ee e2) `union` (fv_ee e3) `union` (fv_qq Q1) `union` (fv_qq Q2)
  | exp_pair P e1 e2 => (fv_qq P) `union` (fv_ee e1) `union` (fv_ee e2)
  | exp_first e1 => (fv_ee e1)
  | exp_second e1 => (fv_ee e1)
  | exp_ref P e1 => (fv_qq P) `union` (fv_ee e1)
  | exp_ref_label P l => (fv_qq P) `union` {}
  | exp_deref e1 => (fv_ee e1)
  | exp_set_ref e1 e2 => (fv_ee e1) `union` (fv_ee e2)
  | exp_upqual P e1 => (fv_qq P) `union` (fv_ee e1)
  | exp_check P e1 => (fv_qq P) `union` (fv_ee e1)
  end.
...
Fixpoint subst_qe (Z : atom) (U : qua) (e : exp) {struct e} : exp :=
  ...
  | exp_app e1 e2 Q => exp_app (subst_qe Z U e1) (subst_qe Z U e2) (subst_qq Z U Q)
  ...
  | exp_let Q e1 e2 => exp_let (subst_qq Z U Q) (subst_qe Z U e1) (subst_qe Z U e2)
  | exp_inl P e1 => exp_inl (subst_qq Z U P) (subst_qe Z U e1)
  | exp_inr P e1 => exp_inr (subst_qq Z U P) (subst_qe Z U e1)
  | exp_case e1 Q1 e2 Q2 e3 => exp_case (subst_qe Z U e1) (subst_qq Z U Q1)
                                  (subst_qe Z U e2) (subst_qq Z U Q2) (subst_qe Z U e3)
...
Fixpoint subst_te (Z : atom) (U : typ) (e : exp) {struct e} : exp :=
  ...
  | exp_app e1 e2 Q => exp_app (subst_te Z U e1) (subst_te Z U e2) Q
  ...
  | exp_let Q e1 e2 => exp_let Q (subst_te Z U e1) (subst_te Z U e2)
  | exp_inl P e1 => exp_inl P (subst_te Z U e1)
  | exp_inr P e1 => exp_inr P (subst_te Z U e1)
  | exp_case e1 Q1 e2 Q2 e3 => exp_case (subst_te Z U e1) Q1
                                  (subst_te Z U e2) Q2 (subst_te Z U e3)
...
Fixpoint subst_ee (z : atom) (u : exp) (R : qua) (e : exp) {struct e} : exp :=
  match e with
  | exp_bvar i => exp_bvar i
  | exp_fvar x => if x == z then u else e
  | exp_abs P V e1 => exp_abs (subst_qq z R P) (subst_qqt z R V) (subst_ee z u R e1)
  | exp_app e1 e2 Q => exp_app (subst_ee z u R e1) (subst_ee z u R e2) (subst_qq z R Q)
  | exp_tabs P V e1 => exp_tabs (subst_qq z R P) (subst_qt z R V) (subst_ee z u R e1)
  | exp_tapp e1 V => exp_tapp (subst_ee z u R e1) (subst_qt z R V)
  | exp_qabs P Q e1 => exp_qabs (subst_qq z R P) (subst_qq z R Q) (subst_ee z u R e1)
  | exp_qapp e1 Q => exp_qapp (subst_ee z u R e1) (subst_qq z R Q)
  | exp_let Q e1 e2 => exp_let (subst_qq z R Q) (subst_ee z u R e1) (subst_ee z u R e2)
  | exp_inl P e1 => exp_inl (subst_qq z R P) (subst_ee z u R e1)
  | exp_inr P e1 => exp_inr (subst_qq z R P)  (subst_ee z u R e1)
  | exp_case e1 Q1 e2 Q2 e3 => exp_case (subst_ee z u R e1) (subst_qq z R Q1)
                                  (subst_ee z u R e2) (subst_qq z R Q2) (subst_ee z u R e3)
  | exp_pair P e1 e2 => exp_pair (subst_qq z R P) (subst_ee z u R e1) (subst_ee z u R e2)
  | exp_first e1 => exp_first (subst_ee z u R e1)
  | exp_second e1 => exp_second (subst_ee z u R e1)
  | exp_ref P e1 => exp_ref (subst_qq z R P) (subst_ee z u R e1)
  | exp_ref_label P l => exp_ref_label (subst_qq z R P) l
  | exp_deref e1 => exp_deref (subst_ee z u R e1)
  | exp_set_ref e1 e2 => exp_set_ref (subst_ee z u R e1) (subst_ee z u R e2)
  | exp_upqual P e1 => exp_upqual (subst_qq z R P) (subst_ee z u R e1)
  | exp_check P e1 => exp_check (subst_qq z R P) (subst_ee z u R e1)
  end.
...
------
  ...
  Case "typ_arrow".
    unfold open_qqt in *.
    pick fresh X.
    eapply (open_qqt_rec_qual_aux T2 0 X)...
  Case "typ_all".
    unfold open_tqt in *.
    pick fresh X.
...
------
  ...
  Case "type_arrow".
    unfold open_qqt in *.
    pick fresh X.
    eapply (open_tqt_rec_qual_aux T2 0 (qua_fvar X))...
  Case "typ_all".
    unfold open_tt in *.
    pick fresh X.
...
Lemma open_te_rec_expr_aux : forall e j (u : atom) i P,
  open_ee_rec j u u e = open_te_rec i P (open_ee_rec j u u e) ->
  e = open_te_rec i P e.
...
(* Lemma open_te_rec_expr_with_qual_aux : forall e j k u i P,
  open_ee_rec j (exp_fvar u) (open_qe_rec k (qua_fvar u) e) = open_te_rec i P (open_ee_rec j u (open_qe_rec k u e)) ->
  e = open_te_rec i P e.
Proof with congruence || eauto using open_tt_rec_qual_aux,
  open_tqt_rec_qual_aux.
  induction e; intros j k u i P H; simpl in *; inversion H; f_equal...
Qed. *)
...
Proof.
  ...
  try solve [
    unfold open_ee in *;
    pick fresh x;
    eapply open_te_rec_expr_aux with (j := 0) (u := x);
    auto
  | unfold open_te in *;
    pick fresh X;
...
Lemma open_ee_rec_expr_aux : forall e j v u i P R,
  i <> j ->
  open_ee_rec j v P e = open_ee_rec i u R (open_ee_rec j v P e) ->
  e = open_ee_rec i u R e.
Proof with congruence || eauto using open_qq_rec_qual_aux, 
  open_qt_rec_qual_aux, open_qqt_rec_qual_aux.
  induction e; intros j v u i P R Neq H; simpl in *; inversion H; f_equal...
  Case "exp_bvar".
    destruct (j===n)... destruct (i===n)...
Qed.

Lemma open_ee_rec_type_aux : forall e j V u i P,
  open_te_rec j V e = open_ee_rec i u P (open_te_rec j V e) ->
  e = open_ee_rec i u P e.
Proof with congruence || eauto using open_qt_rec_type_aux, open_qqt_rec_type_aux.
  induction e; intros j V u i P H; simpl; inversion H; f_equal...
Qed.

Lemma open_ee_rec_qual_aux : forall e j V u i P,
  i <> j -> 
  open_qe_rec j V e = open_ee_rec i u P (open_qe_rec j V e) ->
  e = open_ee_rec i u P e.
Proof with congruence || eauto using open_qqt_rec_qual_aux, open_qt_rec_qual_aux, open_qq_rec_qual_aux.
  induction e; intros j V u i P Neq H; simpl; inversion H; f_equal...
Qed.

Lemma open_ee_rec_expr : forall u e k P,
  expr e ->
  e = open_ee_rec k u P e.
Proof with eauto using open_qq_rec_qual, open_qt_rec_type.
  intros u e k P Hexpr. revert k.
  induction Hexpr; intro k; unfold open_ee in *; simpl; f_equal; auto*;
  try solve [
    unfold open_ee in *;
    pick fresh x;
    eapply open_ee_rec_expr_aux with (j := 0) (v := x);
    auto
  | unfold open_te in *;
    pick fresh X;
  ...
  - eapply open_qqt_rec_type...
Qed.

Lemma subst_ee_fresh : forall (x: atom) u Q e,
  x `notin` fv_ee e ->
  e = subst_ee x u Q e.
Proof with eauto using subst_qq_fresh, subst_qt_fresh, subst_qqt_fresh.
  intros x u Q e; induction e; simpl; intro H; f_equal...
  Case "exp_fvar".
    destruct (a==x)...
    contradict H; fsetdec.
Qed.

Lemma subst_ee_open_ee_rec : forall e1 e2 x u k P R,
  expr u ->
  qual P ->
  subst_ee x u P (open_ee_rec k e2 R e1) =
    open_ee_rec k (subst_ee x u P e2) (subst_qq x P R) (subst_ee x u P e1).
Proof with eauto using subst_qq_open_qq_rec, subst_qt_open_qt_rec, subst_qqt_open_qqt_rec.
  intros e1 e2 x u k P R He Hq. revert k.
...
Lemma subst_ee_open_ee : forall e1 e2 x u P R,
  expr u ->
  qual P ->
  subst_ee x u P (open_ee e1 e2 R) =
    open_ee (subst_ee x u P e1) (subst_ee x u P e2) (subst_qq x P R).
Proof with auto.
  intros.
  unfold open_ee.
...
Lemma subst_ee_open_ee_var : forall (x y:atom) u e P,
  y <> x ->
  expr u ->
  qual P ->
  open_ee (subst_ee x u P e) y y = subst_ee x u P (open_ee e y y).
Proof with congruence || auto.
  intros x y u e P Neq Wu WP.
  unfold open_ee.
...
Lemma subst_te_open_ee_rec : forall e1 e2 Z P k R,
  type P ->
  subst_te Z P (open_ee_rec k e2 R e1) =
    open_ee_rec k (subst_te Z P e2) R (subst_te Z P e1).
Proof with auto.
  induction e1; intros e2 Z P k R Ht; simpl; f_equal...
  - destruct (k === n)...
  - eapply subst_tqt_open_qqt_rec...
  - eapply subst_tt_open_qt_rec...
  - eapply subst_tt_open_qt_rec...
Qed.

Lemma subst_te_open_ee : forall e1 e2 Z P R,
  type P ->
  subst_te Z P (open_ee e1 e2 R) = open_ee (subst_te Z P e1) (subst_te Z P e2) R.
Proof with auto.
  intros.
  unfold open_ee.
...
Lemma subst_te_open_ee_var : forall Z (x:atom) P e R,
  type P ->
  open_ee (subst_te Z P e) x R = subst_te Z P (open_ee e x R).
Proof with auto.
  intros.
  rewrite subst_te_open_ee...
Qed.

Lemma subst_ee_open_te_rec : forall e P z u k R,
  expr u ->
  qual R ->
  subst_ee z u R (open_te_rec k P e) = open_te_rec k (subst_qt z R P) (subst_ee z u R e).
Proof with auto.
  induction e; intros P z u k R He Hq; simpl; f_equal...
  - destruct (a == z)... apply open_te_rec_expr...
  - eapply subst_qqt_open_tqt_rec...
  - eapply subst_qt_open_tt_rec...
  - eapply subst_qt_open_tt_rec...
Qed.

Lemma subst_ee_open_te : forall e P z u R,
  expr u ->
  qual R ->
  subst_ee z u R (open_te e P) = open_te (subst_ee z u R e) (subst_qt z R P).
Proof with auto.
  intros.
  unfold open_te.
...
Lemma subst_ee_open_te_var : forall z (X:atom) u e R,
  expr u ->
  qual R ->
  open_te (subst_ee z u R e) X = subst_ee z u R (open_te e X).
Proof with auto.
  intros z X u e R He Hq.
  rewrite subst_ee_open_te...
Qed.

Lemma subst_ee_intro_rec : forall x e u k R,
  x `notin` fv_ee e ->
  open_ee_rec k u R e = subst_ee x u R (open_ee_rec k (exp_fvar x) (qua_fvar x) e).
Proof with congruence || auto using subst_qq_intro_rec, subst_qqt_intro_rec, subst_qt_intro_rec, subst_qqt_intro_rec.
  induction e; intros u k R Fr; simpl in *; f_equal...
  Case "exp_bvar".
    destruct (k === n)... simpl. destruct (x == x)...
  Case "exp_fvar".
    destruct (a == x)... contradict Fr; fsetdec.
...
Lemma subst_ee_intro : forall x e u P,
  x `notin` fv_ee e ->
  open_ee e u P = subst_ee x u P (open_ee e x x).
Proof with auto.
  intros.
  unfold open_ee.
...
Lemma open_qe_rec_expr_aux : forall e j u i P Q,
  i <> j ->
  open_ee_rec j u Q e = open_qe_rec i P (open_ee_rec j u Q e) ->
  e = open_qe_rec i P e.
Proof with congruence || eauto using open_qq_rec_qual_aux, open_qt_rec_qual_aux, open_qqt_rec_qual_aux.
  induction e; intros j u i P Q Neq H; simpl in *; inversion H; f_equal...
Qed.
...
Proof with eauto.
  induction e; intros j Q i P Heq; simpl in *; inversion Heq;
    ...
    open_qq_rec_qual_aux;
    try solve [rewrite <- H0; eauto];
    try solve [rewrite <- H1; eauto];
    try solve [rewrite <- H2; eauto];
    try solve [rewrite <- H3; eauto].
Qed.
...
Lemma subst_ee_open_qe_rec : forall e P z u k R,
  expr u ->
  qual R ->
  subst_ee z u R (open_qe_rec k P e) = open_qe_rec k (subst_qq z R P) (subst_ee z u R e).
Proof with eauto using subst_qq_open_qq_rec, subst_qt_open_qt_rec, subst_qqt_open_qqt_rec.
  induction e; intros P z u k R He Hq; simpl; f_equal...
  destruct (a == z)... apply open_qe_rec_expr...
Qed.

Lemma subst_ee_open_qe : forall e P z u R,
  expr u ->
  qual R ->
  subst_ee z u R (open_qe e P) = open_qe (subst_ee z u R e) (subst_qq z R P).
Proof with auto.
  intros.
  unfold open_qe.
...
Lemma subst_ee_open_qe_var : forall z (X : atom) u e R,
  X <> z ->
  expr u ->
  qual R ->
  open_qe (subst_ee z u R e) X = subst_ee z u R (open_qe e X).
Proof with auto.
  intros z X u e R Neq He Hq.
  rewrite subst_ee_open_qe...
  simpl.
  destruct (X == z); congruence.
Qed.

Lemma subst_qe_open_ee_rec : forall e1 e2 Z P k R,
  qual P ->
  subst_qe Z P (open_ee_rec k e2 R e1) =
    open_ee_rec k (subst_qe Z P e2) (subst_qq Z P R) (subst_qe Z P e1).
Proof with eauto using subst_qq_open_qq_rec, subst_qt_open_qt_rec, subst_qqt_open_qqt_rec.
  induction e1; intros e2 Z P k R H; simpl; f_equal...
  destruct (k === n)...
Qed.

Lemma subst_qe_open_ee : forall e1 e2 Z P R,
  qual P ->
  subst_qe Z P (open_ee e1 e2 R) = open_ee (subst_qe Z P e1) (subst_qe Z P e2) (subst_qq Z P R).
Proof with auto.
  intros.
  unfold open_ee.
...
Lemma subst_qe_open_ee_var : forall Z (x:atom) P e,
  x <> Z ->
  qual P ->
  open_ee (subst_qe Z P e) x x = subst_qe Z P (open_ee e x x).
Proof with auto.
  intros.
  rewrite subst_qe_open_ee; simpl...
  destruct (x == Z); congruence...
Qed.
...
Lemma subst_te_expr : forall Z P e,
  ...
  type P ->
  expr (subst_te Z P e).
Proof with eauto using subst_tt_type, subst_tqt_type.
  intros Z P e He Hp.
...
Lemma subst_qe_expr : forall Z P e,
  ...
  qual P ->
  expr (subst_qe Z P e).
Proof with eauto using subst_qt_type, subst_qqt_type, subst_qq_qual.
  intros Z P e He Hp.
...
Lemma subst_ee_expr : forall z e1 e2 P,
  expr e1 ->
  expr e2 ->
  qual P ->
  expr (subst_ee z e2 P e1).
Proof with eauto using subst_qt_type, subst_qqt_type, subst_qq_qual.
  intros z e1 e2 P He1 He2 HP.
  induction He1; simpl; auto.
  (* why I can't use try solve here? *)
  - destruct (x == z)...
  - econstructor...
    try instantiate (1 := L `union` singleton z).
    intros.
...
Lemma expr_let_from_body : forall Q e1 e2,
  expr e1 ->
  body_e e2 ->
  qual Q ->
  expr (exp_let Q e1 e2).
Proof.
  intros Q e1 e2 H [J1 J2] Qual.
  pick fresh y and apply expr_let; auto.
Qed.

Lemma body_from_expr_let : forall Q e1 e2,
  expr (exp_let Q e1 e2) ->
  body_e e2.
Proof.
  intros Q e1 e2 H.
...
Lemma expr_case_from_body : forall e1 Q1 e2 Q2 e3,
  expr e1 ->
  qual Q1 ->
  qual Q2 ->
  body_e e2 ->
...
Proof.
  intros e1 Q1 e2 Q2 e3 H WQ1 WQ2 [J1 J2] [K1 K2].
  pick fresh y and apply expr_case; auto.
Qed.

Lemma body_inl_from_expr_case : forall e1 Q1 e2 Q2 e3,
  expr (exp_case e1 Q1 e2 Q2 e3) ->
  body_e e2.
Proof.
  intros e1 Q1 e2 Q2 e3 H.
  unfold body_e.
  inversion H; eauto.
Qed.

Lemma body_inr_from_expr_case : forall e1 Q1 e2 Q2 e3,
  expr (exp_case e1 Q1 e2 Q2 e3) ->
  body_e e3.
Proof.
  intros e1 Q1 e2 Q2 e3 H.
...
Lemma open_ee_body_e : forall e1 e2,
  body_e e1 -> expr e2 -> expr (open_ee e1 e2 qua_top).
Proof.
  intros e1 e2 [L H] J.
  pick fresh x.
